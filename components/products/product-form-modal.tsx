  ...
  useEffect(() => {
    if (isOpen) {
      ...
      fetch('/api/metal-rates')
        .then((r) => r.json())
        .then((d) => {
          if (d.success) setMetalRates(d.data)
        })
    }
  }, [isOpen])

  const isLivePrice = watch('is_live_price')
  const metalType = watch('metal_type')
  const metalWeight = watch('metal_weight')

  useEffect(() => {
    if (isLivePrice && metalType && metalWeight && metalRates[metalType]) {
      const calculatedPrice = metalWeight * metalRates[metalType]
      setValue('selling_price', calculatedPrice)
    }
  }, [isLivePrice, metalType, metalWeight, metalRates, setValue])
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  readOnly={isLivePrice}
                  {...register('selling_price', { valueAsNumber: true })}
                  className={`w-full h-10 px-3 text-sm rounded-xl border border-gray-200 font-bold focus:outline-none focus:ring-2 focus:ring-indigo-500 ${isLivePrice ? 'bg-gray-100 text-gray-500' : 'bg-white text-gray-900'}`}
                />
