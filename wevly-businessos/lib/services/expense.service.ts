...
export class ExpenseService {
  ...
  static async listCategories(session: AppSession) {
    ...
    if (error) {
      throw new Error(`Failed to list expense categories: ${error.message}`);
    }
  ...
  static async createExpense(session: AppSession, payload: CreateExpenseInput) {
    ...
    if (catErr || !category) {
      throw new Error('Expense category not found or unauthorized');
    }
    ...
    if (insertErr || !expense) {
      throw new Error(`Failed to record expense: ${insertErr?.message}`);
    }
