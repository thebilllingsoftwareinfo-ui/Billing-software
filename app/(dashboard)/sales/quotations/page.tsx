...
import { Plus, Search, FileText, Download, ArrowRight, Loader2, CheckCircle2, Edit, Copy, Printer, MessageCircle, Trash2 } from 'lucide-react';
...
export default function QuotationsDirectoryPage() {
  ...
  const handleDuplicateQuotation = (q: any) => {
    const nextNum = `EST-${Date.now().toString().slice(-4)}`;
    const duplicated = {
      ...q,
      id: `quot-${Date.now()}`,
    ...
    toast.success(`Quotation duplicated as draft #${nextNum}!`);
  };

  const handleDeleteQuotation = (q: any) => {
  ...
  const handleConvertToInvoice = (q: any) => {
    toast.success(`Converting quotation ${q.quotation_number} into a Sale Invoice...`);
    router.push(`/sales/invoices/new?quotationId=${q.id}`);
  };

  const handlePrintQuotation = (q: any) => {
    toast.info(`Preparing print for ${q.quotation_number}...`);
    window.print();
  };

  const handleWhatsAppQuotation = (q: any) => {
    const phone = q.customers?.phone || '';
    const clean = phone.replace(/\D/g, '');
    const num = clean.length === 10 ? `91${clean}` : clean;
    const msg = encodeURIComponent(`Dear ${q.customers?.name || 'Customer'}, please review your quotation ${q.quotation_number} for ₹${((q.total_paise || 0) / 100).toFixed(2)}. Thank you!`);
