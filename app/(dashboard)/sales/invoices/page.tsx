...
export default function SalesInvoicesPage() {
  ...
  const handleDuplicateInvoice = (inv: InvoiceRow) => {
    const nextNum = `INV-${Date.now().toString().slice(-4)}`
    ...
    toast.success(`Invoice duplicated as draft #${nextNum}!`)
  }

  const handleDeleteInvoice = (inv: InvoiceRow) => {
    if (!confirm(`Are you sure you want to permanently delete invoice '${inv.invoice_number}'?`)) return
    setInvoices((prev) => prev.filter((i) => i.id !== inv.id))
    toast.success(`Invoice ${inv.invoice_number} deleted`)
  }

  const handleReceivePayment = (inv: InvoiceRow) => {
    const due = inv.balance_due !== undefined ? Number(inv.balance_due) : Number(inv.total_amount) - Number(inv.amount_paid || 0)
    if (due <= 0) {
      toast.info('This invoice is already fully paid!')
      return
    }
    const amountStr = prompt(`Enter payment received against Invoice ${inv.invoice_number} (Due: ₹${due}):`, String(due))
    ...
    setInvoices((prev) =>
      prev.map((i) => {
        ...
        const newPaid = Number(i.amount_paid || 0) + amt
        const newDue = Math.max(0, Number(i.total_amount) - newPaid)
        return {
          ...i,
          amount_paid: newPaid,
          balance_due: newDue,
    ...
    toast.success(`Payment of ₹${amt.toLocaleString('en-IN')} recorded against ${inv.invoice_number}!`)
  }

  const handlePrintInvoice = (inv: InvoiceRow) => {
    toast.info(`Preparing print for ${inv.invoice_number}...`)
    window.print()
  }

  const handlePreviewChallan = (inv: InvoiceRow) => {
    toast.success(`Opening Delivery Challan preview for ${inv.invoice_number}`)
    router.push(`/sales/invoices/${inv.id}?view=challan`)
  }

  const handleConvertReturn = (inv: InvoiceRow) => {
    toast.success(`Generating Sale Return credit note against ${inv.invoice_number}`)
  }

  const handleViewHistory = (inv: InvoiceRow) => {
    toast.info(`Invoice ${inv.invoice_number} created on ${inv.invoice_date}. Status: ${inv.status.toUpperCase()}`)
  }
                          ...
                          <RowActionsMenu
                            items={[
                              ...
                              },
                              {
                                ...
                                icon: Truck,
                                onClick: () => handlePreviewChallan(inv),
                                divider: true,
                              },
                              {
