                    ...
                    <tr key={c.id} className="hover:bg-indigo-50/30 transition-colors group">
                      ...
                      <td className="py-3.5 px-4 text-right relative">
                        <div className="flex items-center justify-end gap-1.5">
                          <Link
                            href={`/customers/${c.id}`}
                            className="p-1.5 rounded-lg border border-gray-200 bg-white hover:bg-gray-50 text-gray-600 hover:text-indigo-600 transition-colors shadow-2xs"
                          ...
                          <button
                            ...
                            className="p-1.5 rounded-lg border border-gray-200 bg-white hover:bg-gray-50 text-gray-600 hover:text-indigo-600 transition-colors cursor-pointer shadow-2xs"
                            title="Edit Customer"
                          >
                          ...
                          <RowActionsMenu
                            items={[
                              ...
                              },
                              {
                                ...
                                icon: FileText,
                                onClick: () => router.push(`/sales/invoices/new?customerId=${c.id}`),
                                divider: true,
                              },
                              {
                                label: 'Record Payment-In',
                                icon: DollarSign,
                                onClick: () => router.push(`/sales/payments?customerId=${c.id}`),
                              },
                              {
                                label: 'Create Estimate / Quotation',
                                icon: FileText,
                                onClick: () => router.push(`/sales/quotations/new?customerId=${c.id}`),
                              },
                              {
                                label: 'Send WhatsApp Statement',
                                icon: MessageCircle,
                                onClick: () => {
                                  toast.success(`WhatsApp reminder opened for ${c.display_name}`)
                                  window.open(`https://wa.me/${(c.phone || c.mobile || '').replace(/\D/g, '')}?text=Dear%20${encodeURIComponent(c.display_name)},%20your%20current%20outstanding%20balance%20is%20${encodeURIComponent(formatCurrency(c.outstanding_balance || 0))}.%20Kindly%20settle%20at%20earliest.`, '_blank')
                                },
                                divider: true,
                              },
                              {
                                label: c.is_active ? 'Archive Customer' : 'Restore Customer',
                                icon: c.is_active ? Archive : RotateCcw,
