...
// Allowed Indian GST Rates (Standard + Special Concessional Rates + Precious Metals)
export const STANDARD_GST_RATES = [0, 0.1, 0.25, 0.5, 1, 1.5, 3, 5, 7.5, 12, 18, 28] as const
export const GST_RATES = [0, 5, 12, 18, 28] as const // Legacy alias
export type GstRate = number

/**
 * Union Territories without legislature where UTGST applies in place of SGST
 * 35: Andaman and Nicobar Islands
 * 04: Chandigarh
 * 26: Dadra and Nagar Haveli and Daman and Diu
 * 38: Ladakh
 * 31: Lakshadweep
 * (Note: Delhi 07, Puducherry 34, and Jammu & Kashmir 01 have legislatures and use SGST)
 */
export const UTGST_STATE_CODES = new Set(['35', '04', '26', '38', '31'])

export function isUtgstTerritory(stateCode: string): boolean {
  if (!stateCode) return false
  return UTGST_STATE_CODES.has(stateCode.trim())
}
...
export interface GstLineInput {
  ...
  gst_rate: number // 0, 0.1, 0.25, 0.5, 1, 1.5, 3, 5, 7.5, 12, 18, 28, or custom
  cess_rate?: number // Cess percentage e.g. 12%
  cess_fixed_amount?: number // Cess fixed amount per unit e.g. ₹400/ton
  cess_amount?: number // Fixed amount or alias per unit
  is_gst_inclusive?: boolean
  tax_category?: 'taxable' | 'nil_rated' | 'exempt' | 'non_gst' | TaxTreatment
 ...
 *   - Intra-state supply: Location of supplier and place of supply are in the SAME state/UT (CGST + SGST or CGST + UTGST).
...
export function calculateCentralGst(input: {
  ...
  const isUtgst = !interState && isUtgstTerritory(seller.state_code)
  ...
  const calculatedLines: CalculatedGstLine[] = items.map((item) => {
    ...
    const cessFixedPerUnit = Number(item.cess_fixed_amount ?? (item as any).cess_amount) || 0
    ...
    const explicitTreatment = item.tax_treatment || (item.tax_category as TaxTreatment)
    const isExempt =
      explicitTreatment === 'exempt' ||
      explicitTreatment === 'nil_rated' ||
      explicitTreatment === 'non_gst' ||
      supply.supply_type === 'exempt' ||
      supply.supply_type === 'nil_rated' ||
      supply.supply_type === 'non_gst'

    const isLineZeroRated =
      isZeroRated ||
      explicitTreatment === 'zero_rated_export' ||
      explicitTreatment === 'zero_rated_sez'

    const treatment: TaxTreatment = isComposition
      ? 'composition'
      : isLineZeroRated
      ? (explicitTreatment as TaxTreatment) || 'zero_rated_export'
      : isReverseCharge || explicitTreatment === 'reverse_charge'
      ? 'reverse_charge'
      : isExempt
      ? (explicitTreatment as TaxTreatment) || 'exempt'
      : 'taxable'

    // Determine effective GST rate
    let effectiveGstRate = Number(item.gst_rate) || 0
    if (isComposition || isLineZeroRated || isExempt) {
      effectiveGstRate = 0
    }
    ...
    if (item.is_gst_inclusive && effectiveGstRate > 0) {
      // ⚠️ REGULATORY COMPLIANCE NOTE:
      // Tax-inclusive pricing back-calculates taxable value:
    ...
    // Calculate Cess: Percentage-based on taxable amount + Fixed per-unit
    let lineCess = 0
    if (cessRate > 0 && !isComposition && !isExempt && !isLineZeroRated) {
      lineCess += Math.round(taxable * (cessRate / 100) * 100) / 100
    }
    if (cessFixedPerUnit > 0 && !isComposition && !isExempt && !isLineZeroRated) {
      lineCess += Math.round(qty * cessFixedPerUnit * 100) / 100
    }
    lineCess = Math.round(lineCess * 100) / 100
    ...
    if (effectiveGstRate > 0) {
      if (interState || supply.supply_type === 'zero_rated_export' || (supply.supply_type as any) === 'sez_with_tax') {
        ...
        // UTGST territory: Split between CGST (50%) & UTGST (50%)
        cgstRate = effectiveGstRate / 2
        utgstRate = effectiveGstRate / 2
        cgstAmt = Math.round((gstTax / 2) * 100) / 100
        utgstAmt = Math.round((gstTax - cgstAmt) * 100) / 100 // Remainder allocated to UTGST to prevent 1-paise gap
      } else {
        // Standard State: Split between CGST (50%) & SGST (50%)
        cgstRate = effectiveGstRate / 2
        sgstRate = effectiveGstRate / 2
        cgstAmt = Math.round((gstTax / 2) * 100) / 100
        sgstAmt = Math.round((gstTax - cgstAmt) * 100) / 100 // Remainder allocated to SGST to prevent 1-paise gap
      }
    }

    const totalLineTax = Math.round((gstTax + lineCess) * 100) / 100
    const lineTotal = item.is_gst_inclusive
      ? Math.round((afterDisc + (cessFixedPerUnit * qty)) * 100) / 100
      : Math.round((taxable + totalLineTax) * 100) / 100
    ...
    return {
      ...
      utgst_amount: utgstAmt,
      igst_rate: igstRate,
      igst_amount: igstAmt,
      cess_rate: cessRate,
      cess_amount: lineCess,
      total_tax: totalLineTax,
  ...
  const totalTaxAmount = Math.round((totalCgst + totalSgst + totalUtgst + totalIgst + totalCess) * 100) / 100
  ...
  for (const line of calculatedLines) {
    ...
    entry.utgst_amount = Math.round((entry.utgst_amount + line.utgst_amount) * 100) / 100
    entry.igst_amount = Math.round((entry.igst_amount + line.igst_amount) * 100) / 100
    entry.cess_amount = Math.round((entry.cess_amount + line.cess_amount) * 100) / 100
  ...
  return {
    ...
    utgst_amount: Math.round(totalUtgst * 100) / 100,
...
export function calculateInvoiceTotals(lineItems: any[], invoiceDiscountPaise: number = 0) {
  ...
  return {
    ...
    cess_paise: Math.round(calc.cess_amount * 100),
